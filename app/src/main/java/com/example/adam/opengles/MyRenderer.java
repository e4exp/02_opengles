...
public class MyRenderer implements GLSurfaceView.Renderer {
    ...
    private final FloatBuffer mTriangleVertices2;  // 頂点バッファ
    ...
    MyRenderer() {
        ...
        mTriangleVertices=setBuffer(triangleVerticesData);
        mTriangleVertices2=setBuffer(triangleVerticesData);


    }

    FloatBuffer setBuffer(float[] data){
        FloatBuffer fb = ByteBuffer.allocateDirect(data.length * mBytesPerFloat)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        fb.put(data).position(0);  // データをバッファへ
        return fb;
    }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        float x=mRotX % 100;
        float y=mRotY % 100 ;
        float angleInDegreesX=360.0f /100.0f * x;
        float angleInDegreesY=360.0f /100.0f * y;

        // ワールド行列に対して回転をかける
        Matrix.setIdentityM(mModelMatrix, 0);  // 単位行列でリセット
        //Matrix.rotateM(mModelMatrix, 0, angleInDegrees, 0.0f, 0.0f, 1.0f);  // 回転行列
        Matrix.rotateM(mModelMatrix, 0, angleInDegreesX, 0.0f, 1.0f, 0.0f);  // 回転行列
        Matrix.rotateM(mModelMatrix, 0, angleInDegreesY, 1.0f, 0.0f, 0.0f);  // 回転行列
        ...
        //translate
        Matrix.translateM(mModelMatrix, 0, 1.0f, 0.0f, 0.0f);  // 回転行列
        //triangle2
        drawTriangle(mTriangleVertices2);
    ...
    //can only draw less than triangle. rectangle doesnt exist.
    // 三角形を描画する
